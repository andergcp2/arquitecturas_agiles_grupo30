...
def guardarTransaccion(tipo_transaccion ,valor ,id_dueno, id_creador, id_apuesta=''):
...
class VistaSignIn(Resource):
    ...
    def post(self):
        ...
        usuario_creado = db.session.query(Usuario).filter(Usuario.usuario.like(request.json["usuario"]), Usuario.contrasena.like(request.json["contrasena"])).first()
        guardarTransaccion("SALDO_PROMO", nuevo_usuario.saldo_promocional, usuario_creado.id, usuario_creado.id, '')
        db.session.commit()
...
class VistaCarrerasUsuario(Resource):
    ...
    def post(self, id_usuario):
        if (request.json["tipo_evento"] == 'EVENTO_CON_MARCADOR' and len(request.json["competidores"]) != 3):
            return 'El evento con marcador requiere tres competidores', 400
    ...
    def get(self, id_usuario):
        ...
        if "ADMIN" == usuario.role:
            ...
            carreras = db.session.query(Carrera).select_from(Apuesta).join(Carrera.apuestas).filter(Apuesta.id_apostador==id_usuario).all()
            return [carrera_schema.dump(ca) for ca in carreras]
...
class VistaCarrera(Resource):
    ...
    def put(self, id_carrera):
        ...
        carrera.tipo_evento = request.json.get("tipo_evento", carrera.tipo_evento)
...
class VistaApuestas(Resource):
    ...
    def post(self):
        ...
        if valor_apostado>=5000: 
            if total_apostador >= valor_apostado:
                ...
                #Transaccion para apostador
                self.guardarTransaccion("APUESTA", (valor_apostado*-1), apostador.id, user_id, newapuesta['id'])
                #Transaccion para administrador
                self.guardarTransaccion("APUESTA", valor_apostado, admin.id, user_id, newapuesta['id'])
                return newapuesta
            else:
    ...
    def guardarTransaccion(self, tipotran, valor, id_dueno, idusuario, idapuesta):
...
class VistaTransacciones(Resource):
    ...
    def get(self):
        ...
        usuario = Usuario.query.get_or_404(user_id)
        #return usuario_schema.dump(usuario)
        transacciones = [transaccion_schema.dump(tr) for tr in Transaccion.query.filter(Transaccion.id_dueno == user_id).order_by(desc(Transaccion.fecha_transaccion)) ]
...
class VistaCuenta(Resource): 

    def put(self, id_usuario):
        usuario = Usuario.query.get_or_404(id_usuario)
        valor_deposito=request.json.get("valor_deposito")
        usuario.saldo = usuario.saldo + valor_deposito
        db.session.commit()
        guardarTransaccion("DEPOSITO", valor_deposito, id_usuario, id_usuario)
        usuario_schema.dump(usuario)
        return {"mensaje": "Se hizo un deposito a su cuenta de manera exitosa", "nuevo_saldo": usuario.saldo}

class VistaKong(Resource):
    def get(self):
        usr = Usuario.query.filter(Usuario.usuario == 'admin').first()
        if usr is None:
            return "El usuario no existe", 404
        else:
            token = create_access_token(identity=usr.id, additional_claims={"role": usr.role, "user": usr.usuario, "username": usr.nombre})
