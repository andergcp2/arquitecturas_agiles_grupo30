...
class Carrera(db.Model):
    ...
    tipo_evento = db.Column(db.Enum("CARRERA", "EVENTO_CON_MARCADOR", name='tipoEvento'))
    ganancia_casa = db.Column(db.Float, default=0, nullable=True)
...
class Usuario(db.Model):
    ...
    saldo_promocional = db.Column(db.Integer)
    saldo =  db.Column(db.Integer)
    role = db.Column(db.Enum("ADMIN", "GAMBLER", name='RoleUser'))
...
class Transaccion(db.Model):
    ...
    tipo_transaccion = db.Column(db.Enum("APUESTA", "DEPOSITO", "GANANCIA", "RETIRO", "SALDO_PROMO", name='TipoTransaccion'))                        
    valor = db.Column(db.Integer)
    id_dueno = db.Column(db.Integer, db.ForeignKey('usuario.id'))
    id_creador = db.Column(db.Integer, db.ForeignKey('usuario.id'))
    id_apuesta = db.Column(db.Integer, db.ForeignKey('apuesta.id'), nullable=True)
    creador = db.relationship('Usuario', foreign_keys=[id_dueno])
    dueno = db.relationship('Usuario', foreign_keys=[id_creador])
